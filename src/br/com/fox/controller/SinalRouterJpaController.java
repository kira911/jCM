...
public class SinalRouterJpaController implements Serializable {
    ...
    public List<SinalRouter> findSinalRouter(String tpEvento, Integer limit, Integer idcliente) {
        EntityManager em = getEntityManager();
        try {
            Query q = em.createQuery("SELECT s FROM SinalRouter s "
                    + "WHERE s.org like '" + tpEvento + "' AND "
                    + "s.codCli.idcliente = " + idcliente + " "
                    + "ORDER BY 1 DESC");
    ...
    public List<SinalRouter> findSinalRouterNaoAtendido() {
        EntityManager em = getEntityManager();
        try {
            Query q = em.createQuery("SELECT s FROM SinalRouter s "
                    + "WHERE s.status = 'NÃO ATENDIDO'");
    ...
    public List<SinalRouter> findSinalRouterEmEspera() {
        EntityManager em = getEntityManager();
        try {
            Query q = em.createQuery("SELECT s FROM SinalRouter s "
    ...
    public List<SinalRouter> findSinalRouterObservacao() {
        EntityManager em = getEntityManager();
        try {
            Query q = em.createQuery("SELECT s FROM SinalRouter s "
                    + "WHERE s.status = 'OBSERVAÇÃO'");
